    ...
    if ($usertype == "Student" || $usertype == "Editor") {
        ...
        <html>
            <head>
                ...
                <meta charset="UTF-8">
                <title>Photographer Committee join request</title>
            ...
            <body>
                ...
                $flag = FALSE;
                $success = FALSE;
                $query = "select * from tblstudent where isEditor=4 and contactNo=" . $_SESSION["username"];
                ...
                if (isset($_POST["btnSubmit"])) {
                    $query = "update tblstudent set isEditor=4 where contactNo=" . $_SESSION["username"];
                    if ($con->query($query)) {
                        $query = "select * from tblnotification where type=5";
                        $result = $con->query($query);
                        if ($result->num_rows == 0) {
                            $query = "insert into tblnotification(text,isReaded,otherData,type) value('Student request to join photographer committee',0,'photographerRequest.php',5)";
                            $con->query($query);
                        }
                        $flag = TRUE;
                        $success = TRUE;
                                ...
                                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" enctype="multipart/form-data">
                                    ...
                                    <div class="row contact-wrap">
                                        ...
                                        <div class="col-5 col-sm-5 col-md-5 col-lg-5 col-xl-5">
                                            <center>
                                                <h5>Do you want join photographer committee then apply here</h5>
                                            </center>
                                        </div>
                                    </div>
                                    <div class="row contact-wrap">
